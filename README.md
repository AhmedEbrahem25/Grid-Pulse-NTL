...
[![Model](https://img.shields.io/badge/AI-multi--factor%20(not%20temp--only)-8a2be2)](#-the-model-is-multi-factor)
...
| AI | **scikit‑learn** (RandomForest regressor + IsolationForest) | trains in seconds on physics‑seeded synthetic data |
...
flowchart TD
    ...
    C["③ Harmonics / PF<br/>theft_signature_score"] --> D
    E["④ Temporal Anomaly<br/>IsolationForest score"] --> D
    D["⑤ Decision fusion"] --> V["Verdict<br/>label · theft_probability · theft_estimate_w · severity · XAI"]
```
...
| ③ Harmonics / PF | `detection/harmonics.py` | illegal hooks distort the waveform & drop power factor |
| ④ Temporal anomaly | `detection/anomaly.py` | catches repeating unexplained draws (e.g. 02:00–06:00 workshop hooks) |
| ⑤ Decision fusion | `detection/decision.py` | fuses all four → label, confidence, theft estimate, reasons |
...
├─ ml/            generate_dataset.py · train.py · artifacts/ (tech_loss.pkl · iforest.pkl · meta.json)
...
`1D‑CNN harmonic classifier` · `TinyML on‑device inference (send only on theft → −99% bandwidth)` · `theft‑location triangulation across feeders` · `load‑forecasting to pre‑empt overload / load‑shedding` · `smart‑contactor auto‑cutoff` · `hardware anti‑tamper crypto` · `federated learning across regions`.
...
`INLINE_DETECTION=true` (default) runs detection inside the API — simplest and demo‑safe. Set it `false` to route through the Celery worker (the microservices story); verdicts are published back over Redis pub/sub. Other knobs in `.env.example`: `INGEST_TOKEN`, `RESIDUAL_ALERT_W` (unexplained watts before suspicion), `HEALTHY_PF`, `REDIS_URL`.
